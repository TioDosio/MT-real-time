...
class SimplePersonFloorDetector:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        if int(box.cls) == 0 and float(box.conf) > 0.5:
                            ...
                            print(f"Detected person with confidence {box.conf} at [{x1}, {y1}, {x2}, {y2}]")
                            ...
                            if floor_coords is not None:
                                ...
                                rospy.loginfo(f"Person at floor: x={floor_coords[0]:.2f}m, "
                                            f"y={floor_coords[1]:.2f}m")
    ...
    def pixel_to_floor_coordinates(self, pixel_x, pixel_y, image_shape):
        ...
        """
        print("input arguments:", pixel_x, pixel_y, image_shape)
        try:
            ...
            print(f"Angle_y: {angle_y} radians, {np.degrees(angle_y)} degrees")
            ...
            print(f"Downward angle: {downward_angle} radians, {np.degrees(downward_angle)} degrees")
            
            if downward_angle <= 0:  # Looking up or horizontal, won't hit ground
                print("Looking up or horizontal - no ground intersection")
                return None
            
            # Distance forward to ground intersection
            ground_distance = self.camera_height / np.tan(downward_angle)
            print(f"Ground distance: {ground_distance}")
            ...
            print(f"Final floor coords: {floor_coords}")
            return floor_coords
